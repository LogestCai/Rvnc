define(["jquery", "tableSelect", "ckeditor","inputTags"], function ($, tableSelect, undefined,inputTags) {
    ...
    var admin = {
        ...
        table: {
            render: function (options) {
                ...
                //options.operat = options.operat || ['edit', 'delete'];
                ...
                $.each(data, function (i, v) {
                    if (v === 'refresh') {
                        ...
                        if (admin.checkAuth('delete', elem)) {
                            toolbarHtml += '<button class="layui-btn layui-btn-sm layui-btn-danger" data-url="' + init.delete_url + '" data-table-delete="' + tableId + '"><i class="fa fa-trash-o"></i><span class="layui-hide-xs"> 删除</span></button>';
                        }
                    } else if (v === 'export') {
            ...
            buildToolbarHtml: function (toolbar, tableId) {
                ...
                if(toolbar.type !== 'btn'){
                    //html = '<a ' + formatToolbar.class + 'href="'+toolbar.url+'"'+ formatToolbar.tableId + '>' + formatToolbar.icon + formatToolbar.text + '</a>';
                    html = '<a ' + formatToolbar.class + formatToolbar.method + formatToolbar.extend + '>' + formatToolbar.icon + formatToolbar.text + '</a>';
            ...
            buildOperatHtml: function (operat) {
                ...
                operat.type = operat.type || '_black';
                ...
                } else if(operat.method === 'jump') {
                    // jump 配合extend使用 如 extend="_target" //
                    formatOperat.method = formatOperat.method !== '' ? 'href="' + admin.url(formatOperat.url) + '" title="' + formatOperat.title + '"  target="'+operat.type+'"' : '';
                }else if(operat.method === 'layuimini-href') {
                    // 在minitab选项卡中打开
                    formatOperat.method = formatOperat.method !== '' ? 'layuimini-content-href="' + admin.url(formatOperat.url) + '" data-title="' + formatOperat.title + '" ' : '';
                }
                else{
            ...
            image: function (data, option) {
                ...
                if (value === undefined) {
                    ...
                    var values = value.split(option.imageSplit),
                        valuesHtml = [];
                   // values.forEach((value, index) => {//Es6语法
        ...
        api: {
            ...
            upload: function () {
                //根据data-upload获取到需要文件上传的元素列表
                var uploadList = document.querySelectorAll("[data-upload]");
                //根据data-upload-select获取到需要文件选择的元素列表
                ...
                if (uploadList.length > 0) {
                    $.each(uploadList, function (i, v) {
                        ...
                        if(uploadUrl==undefined||uploadUrl==''){
                            uploadUrl=admin.url(init.upload_url);
                        }
                        // 监听上传事件
                        upload.render({
                            elem: this,
