...
### 启动vnc代理
echo "===================正在启动vnc代理容器=================================="
docker run -itd --name vnc-proxy  -p 6080:6080 -v $work_dir/noVNC:/data royeecai/novnc-websockify   --web /data :6080 --token-plugin TokenRedis --token-source "$1":6379
...
sed -i "s/host_ip=10.87.230.199/host_ip=$1/g" ./php/www/.env
